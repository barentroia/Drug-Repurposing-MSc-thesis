...
generateDiagPlots<-function(accession){
  ...
  RLE(celfiles.qc, main=paste("RLE ",accession,sep=""),names=rep("",length(ph@data$Names)))
  dev.off()
  cat("\nPlotting NUSE")
  png(filename = paste("../Plots",accession,"nuse_beforenorm.png",sep="/"),width=800,height=484)
  NUSE(celfiles.qc, main=paste("NUSE ",accession,sep=""),names=rep("",length(ph@data$Names)))
  dev.off()
  
  cat("\nComputing quality control measures")
  celfiles.qcmet = qc(celfiles)
  scfact=sfs(celfiles.qcmet)
