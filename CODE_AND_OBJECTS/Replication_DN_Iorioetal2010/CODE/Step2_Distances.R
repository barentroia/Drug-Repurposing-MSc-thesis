...
#' retrieveSignature returns the top 250 and bottom 250 genes. These are the signature of each drug
#'
#' @param PRLa The PRL (prototype ranekd list) of a given drug
#' @return signature - a list where Top contains top 250 genes in the PRL and Bottom the bottom 250 ones.
...
computeDist=function(PRLa,PRLb){
  ...
  dist=maxESDistance(TESa,TESb)
  
  return(dist)
}

#' buildDistMatrix produces the distance matrix corresponding to the drugs whose PRLs are in DRUG_PRLs
#' 
...
print("The computation of the distances was parallelized. The current code uses 8 cores and takes approximately 8 hours to compute the distances between all pairs of the 1309 CMap drugs")
distMatrix=buildDistMatrix(DRUG_PRLs)
write.table(distMatrix,"../OUTPUT/DrugDistances.txt",sep="\t")
